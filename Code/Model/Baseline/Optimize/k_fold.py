...
# --- 1. CẤU HÌNH HỆ THỐNG (Sửa lỗi seed) ---
...
class UltimateModelV9(nn.Module):
    ...
    def forward(self, ids, mask, img):
        ...
        T2I, _ = self.t2i(H_T, H_I, H_I); H_T = self.normT(H_T + T2I)
        I2T, _ = self.i2t(H_I, H_T, H_T, key_padding_mask=(mask == 0)); H_I = self.normI(H_I + I2T)
...
def save_plots(exp_name, train_losses, val_f1s, y_true, y_pred, y_probs):
    sns.set_theme(style="whitegrid"); plt.figure(figsize=(20, 12))
    plt.subplot(2, 2, 1); plt.plot(train_losses, label='Train Loss', color='b'); plt.title("Loss History")
    plt.subplot(2, 2, 2); plt.plot(val_f1s, label='Val F1 Macro', color='g'); plt.title("F1 Macro History")
    ...
    plt.plot([0, 1], [0, 1], 'k--'); plt.legend(); plt.savefig(f"{exp_name}_metrics.png"); plt.show()

# --- 5. MAIN TRAINING LOOP (BỌC K-FOLD) ---
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(full_df, full_df['label'])):
        ...
        train_loader = DataLoader(torch.utils.data.Subset(MVSADataset(CFG.CSV_PATH, CFG.IMG_DIR, transform), train_idx), batch_size=CFG.BATCH_SIZE, shuffle=True)
        val_loader = DataLoader(torch.utils.data.Subset(MVSADataset(CFG.CSV_PATH, CFG.IMG_DIR, transform), val_idx), batch_size=CFG.BATCH_SIZE)
        ...
        for stage, epochs, lr in [("S1", CFG.STAGE_1_EPOCHS, CFG.LEARNING_RATE_S1), ("S2", CFG.STAGE_2_EPOCHS, CFG.LEARNING_RATE_S2)]:
            ...
            for ep in range(epochs):
                ...
                for b in tqdm(train_loader, desc=f"V9-{stage}-Ep{ep+1}"):
                    ...
                    scaler.scale(loss).backward(); scaler.step(opt); scaler.update(); total_loss += loss.item()
