...
class CFG:
    ...
    CSV_PATH = "/kaggle/input/datasets/nhatnhat26/cpcsvf/mvsa_clean/mvsa.csv"
    IMG_DIR = "/kaggle/input/datasets/nhatnhat26/mvsa-dataset/MVSA/data"
...
class MVSADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "tokens": self.tokenizer.convert_ids_to_tokens(enc["input_ids"].squeeze())
        }
...
class UltimateModelV9(nn.Module):
    ...
    def forward(self, ids, mask, img):
        ...
        # Cross-Attention 
        T2I, t2i_attn = self.t2i(H_T, H_I, H_I) # Text focus vào Image
        H_T = self.normT(H_T + T2I)
        
        I2T, i2t_attn = self.i2t(H_I, H_T, H_T, key_padding_mask=(mask == 0)) # Image focus vào Text
        ...
        return logits, z, (t2i_attn, i2t_attn)
...
def save_plots(exp_name, train_losses, val_f1s, y_true, y_pred, y_probs):
    ...
    plt.subplot(2, 2, 1); plt.plot(train_losses, label='Train Loss'); plt.title("Loss History"); plt.legend()
    plt.subplot(2, 2, 2); plt.plot(val_f1s, label='Val F1', color='g'); plt.title("F1 History"); plt.legend()
    plt.subplot(2, 2, 3); cm = confusion_matrix(y_true, y_pred)
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Neg','Neu','Pos'], yticklabels=['Neg','Neu','Pos'])
    plt.title("Confusion Matrix")
    plt.subplot(2, 2, 4); y_true_bin = label_binarize(y_true, classes=[0, 1, 2])
    for i, label in enumerate(['Neg', 'Neu', 'Pos']):
        fpr, tpr, _ = roc_curve(y_true_bin[:, i], y_probs[:, i])
        plt.plot(fpr, tpr, label=f'{label} (AUC = {auc(fpr, tpr):.2f})')
    plt.plot([0, 1], [0, 1], 'k--'); plt.title("ROC Curves"); plt.legend()
    plt.savefig(f"{exp_name}_metrics.png"); plt.show()

def visualize_attention_maps(model, dataset, idx):
    model.eval()
    data = dataset[idx]
    ids = data["input_ids"].unsqueeze(0).to(CFG.DEVICE)
    mask = data["attention_mask"].unsqueeze(0).to(CFG.DEVICE)
    img = data["image"].unsqueeze(0).to(CFG.DEVICE)
    
    with torch.no_grad():
        _, _, (t2i_attn, i2t_attn) = model(ids, mask, img)
    
    v_attn = t2i_attn[0, 0, 1:].cpu().numpy().reshape(14, 14)
    # Textual Weights: 
    t_attn = i2t_attn[0].mean(0).cpu().numpy()

    fig, ax = plt.subplots(1, 2, figsize=(15, 6))
    ax[0].imshow(data["orig_image"])
    ax[0].imshow(v_attn, cmap='jet', alpha=0.4, extent=(0, 224, 224, 0), interpolation='bilinear')
    ax[0].set_title("Visual Attention Map")
    
    non_pad = sum(data["attention_mask"]).item()
    sns.barplot(x=data["tokens"][:non_pad], y=t_attn[:non_pad], ax=ax[1])
    ax[1].set_title("Textual Attention Weights")
...
def main():
        ...
        for ep in range(epochs):
            ...
            for b in tqdm(train_loader, desc=f"V9-{stage}-Ep{ep+1}"):
                ...
                with torch.amp.autocast('cuda'):
                    logits, z, _ = model(ids, mask, img)
                    loss = loss_fn(logits, y) + (0.05 * supcon_loss(z, y) if stage == "S1" else 0)
    ...
    visualize_attention_maps(model, dataset, indices[int(0.8*n)])
